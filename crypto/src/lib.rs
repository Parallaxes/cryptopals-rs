...
fn decrypt_cbc(ciphertext: &[u8], key: &[u8], iv: &[u8]) -> Result<Vec<u8>, AesError>{
    if ciphertext.len() % AES128_BLOCK_SIZE != 0 {
        return Err(AesError::InvalidBlockSize);
    }
    ...
    for block in ciphertext.chunks_exact(AES128_BLOCK_SIZE) {
        let decrypted_block = decrypt_block_raw(block, key)?;
...
fn encrypt_cbc(plaintext: &[u8], iv: &[u8], key: &[u8]) -> Result<Vec<u8>, AesError>{
    ...
    for block in plaintext.chunks_exact(AES128_BLOCK_SIZE) {
        ...
        let ciphertext_block = encrypt_block_raw(&xor_block, key)?;
